function results = run_double_pendulum(Hz, u, u_test, noise)
...
%   results  - Struct with all generated variables (Data, dData, models, etc.)
    ...
    dt = 1/Hz;
    Tf=15; tspan=0:dt:Tf;
    T_test=25; tspan_test=0:dt:T_test;
...
[dData_Es,Data_Es]=Get_Sim_Data(@(t,z,inp)Sindy_ODE_RHS(t,z,inp),state0,u,tspan,Noise_test,Control,Shuffle);
if ProvidedData ~= 1
[dData_test,Data_test]=Get_Sim_Data(@(t,y,inp)DouPenODE(t, y, inp, l1, l2, m1, m2, Jm1, Jm2, b1, b2, tau1, tau2, tanh_k,0),state0,u,tspan,Noise_test,Control,Shuffle);
else
    Data_test = Data(1:length(tspan),:);
    dData_test = dData(1:length(tspan),:);
end
%% Save the result
...
close all
figure(1)
plot(tspan,Data_test(:,1),'linewidth',4.5,'Color','black')
hold on
plot(tspan,Data_Es(:,1),'linewidth',4.5,'linestyle','--','color','blue')
% title('Validation')
% xlabel('Time $(t)$')
% ylabel('$\theta_1$')
...
%
figure(2)
plot(tspan,Data_test(:,2),'linewidth',4.5,'Color','black')
hold on
plot(tspan,Data_Es(:,2),'linewidth',4.5,'linestyle','--','color','blue')
% title('Validation')
% xlabel('Time $(t)$')
% ylabel('$\theta_2$')
% legend('Actual Dynamics','Approximated Dynamics')
...
print('-depsc2', '-loose', 'Figures/DoublePendulum_PhasePlot_dTheta1_vs_dTheta2.eps');
    ...
    %% Collect outputs into struct
    results = struct();
    results.Data = Data;
    results.dData = dData;
    results.Data_test = Data_test;
    results.dData_test = dData_test;
    results.tspan = tspan;
    results.tspan_test = tspan_test;
    % Optionally add Xi, ODEs, ODE_Best, Score, etc. if Sindy is run
    if exist('Xi','var'), results.Xi = Xi; end
    if exist('ODEs','var'), results.ODEs = ODEs; end
    if exist('ODE_Best','var'), results.ODE_Best = ODE_Best; end
    if exist('Score','var'), results.Score = Score; end
end
